...
from PyQt6.QtCore import (
    QPointF, QRectF, QSize, Qt, QTimer, pyqtSignal,
)
from PyQt6.QtGui import (
    QBrush, QColor, QConicalGradient, QDragEnterEvent, QDropEvent, QFont,
    QIcon, QKeySequence, QPainter, QPen, QPixmap, QRadialGradient, QShortcut,
)
...
class C:
    # ── Neon / glass palette — cyan primary + indigo secondary, matches the Web UI ──
    BG        = "#05070d"
    PANEL     = "#070b14"
    PANEL2    = "#0a0f1c"
    BORDER    = "#16324a"
    BORDER_B  = "#22d3ee"
    BORDER_A  = "#1f4f6b"
    PRI       = "#22d3ee"   # neon cyan (matches Web UI accent)
    PRI_DIM   = "#0e7490"
    PRI_GHO   = "#06262f"
    ACC       = "#ff6b00"
    ACC2      = "#6366f1"   # indigo secondary (matches Web UI accent-2)
    GREEN     = "#34d399"
    GREEN_D   = "#10b981"
    RED       = "#fb7185"
    MUTED_C   = "#fb7185"
...
# ── Hand-gesture feedback styling (HUD overlay) ──────────────────────────────
# Each known gesture maps to a neon color + the number of "finger dots" drawn
# in its emblem (fully geometric → no font/emoji dependency, always renders).
_GESTURE_STYLE: dict[str, tuple[str, int]] = {
    "OPEN_PALM":   (C.GREEN,   5),
    "FIST":        (C.MUTED_C, 0),
    "POINT":       (C.PRI,     1),
    "TWO_FINGERS": (C.ACC2,    2),
    "PINCH":       (C.ACC,     2),
    "WAVE":        (C.GREEN,   3),
    "CLAP":        (C.ACC,     2),
    "PEACE":       (C.ACC2,    2),
...
def _lerp_color(a: QColor, b: QColor, t: float) -> QColor:
    """Linearly interpolate two QColors (t in [0,1]). Returns a new QColor."""
    t = max(0.0, min(1.0, t))
    return QColor(
...
def _state_target_color(state: str, muted: bool) -> QColor:
    """Resolve the target orb/halo color for a given JARVIS state."""
    if muted:
...
# ── Typography ────────────────────────────────────────────────────────────
# A refined monospace stack: prefers a crisp UI mono font, falls back cleanly
# on every OS (Windows → Consolas, macOS → SF Mono, Linux → DejaVu/JetBrains).
MONO = "'SF Mono', 'JetBrains Mono', 'DejaVu Sans Mono', Consolas, 'Courier New', monospace"


def _m(size: int, bold: bool = False) -> QFont:
    return QFont(MONO, size, QFont.Weight.Bold if bold else QFont.Weight.Normal)


def markdown_to_html(text: str) -> str:
    """
    Tiny Markdown → HTML renderer for the content panel (no external deps).
    Supports headers, **bold**, *italic*, `code`, [links](url), and bullet lists.
    """
    ...
    def inline(s: str) -> str:
        ...
        s = _re.sub(
            r"`([^`]+)`",
            r"<code style='background:#001a22;color:#7ff;padding:0 4px;"
            r"border-radius:3px;font-size:9pt;'>\1</code>",
            s,
        )
        ...
        s = _re.sub(
            r"\[([^\]]+)\]\((https?://[^)]+)\)",
            r"<a href='\2' style='color:#00d4ff;text-decoration:none;'>"
            r"&#128279; \1</a>",
            s,
        )
        return s
    ...
    for raw in text.split("\n"):
        ...
        if h:
            ...
            lvl = len(h.group(1))
            out.append(
                f"<h{lvl} style='color:#00d4ff;margin:3px 0 2px;font-weight:bold;"
                f"font-family:{MONO};'>{inline(h.group(2))}</h{lvl}>"
            )
            continue
        if _re.match(r"^[-*]\s+", line):
            if not list_open:
                ...
                list_open = True
            out.append(
                "<li style='margin:1px 0;'>" + inline(_re.sub(r"^[-*]\s+", "", line)) + "</li>"
            )
            continue
        numbered = _re.match(r"^\d+\.\s+(.*)$", line)
        if numbered:
            close_list()
            out.append(
                "<p style='margin:1px 0;'><b style='color:#00d4ff;'>"
                + inline(numbered.group(1)) + "</b></p>"
            )
            continue
        close_list()
        out.append("<p style='margin:1px 0;'>" + inline(line) + "</p>")

    close_list()
    return (
        "<div style='font-family:" + MONO + ";font-size:9pt;line-height:1.4;"
        "color:#8ffcff;'>" + "".join(out) + "</div>"
    )
...
# Push the HUD's live system readings into the process-wide metrics registry so
# the Web UI health panel (/metrics) can surface real CPU/MEM/GPU/UPTIME data.
try:
    from core.observability import metrics as _health_metrics
...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "J.A.R.V.I.S", parent=None):
        ...
        # ── Hand-control feedback overlays ──
        self._gesture_fx: list[dict] = []   # active gesture badges (name, color, dots, t0, conf)
        self._claps: list[float] = []        # expanding clap-ripple radii
    ...
    # ── Hand-control feedback API ───────────────────────────────────────────
    def gesture_feedback(self, name: str, confidence: float = 0.0) -> None:
        """Queue an animated gesture badge (thread-safe — called from UI signals)."""
        try:
            key = (name or "").upper().strip()
            color_hex, dots = _GESTURE_STYLE.get(key, (C.PRI, 3))
            self._gesture_fx.append({
                "name": name,
            ...
            # Cap concurrent effects to avoid clutter / unbounded growth
            if len(self._gesture_fx) > 4:
                self._gesture_fx = self._gesture_fx[-4:]
        except Exception:
            pass

    def clap_feedback(self) -> None:
        """Queue an expanding clap ripple from the orb center."""
        try:
    ...
    def _step(self):
        ...
        speeds = [1.3, -0.9, 2.0] if self.speaking else ([1.0, -0.7, 1.5] if self.state == "READY" else [0.55, -0.35, 0.9])
        for i, spd in enumerate(speeds):
            self._rings[i] = (self._rings[i] + spd) % 360

        _scan_spd = 3.0 if self.speaking else (2.0 if self.state == "READY" else 1.3)
        _scan2_spd = -2.0 if self.speaking else (-1.2 if self.state == "READY" else -0.75)
        self._scan  = (self._scan  + _scan_spd) % 360
        self._scan2 = (self._scan2 + _scan2_spd) % 360

        fw  = min(self.width(), self.height())
        lim = fw * 0.74
        spd = 4.2 if self.speaking else (3.0 if self.state == "READY" else 2.0)
        ...
        self._col_cur = _lerp_color(self._col_cur, self._col_tgt, 0.12)
    ...
    def paintEvent(self, _):
        ...
        # radial vignette — brighter core, deep edges
        grad = QRadialGradient(cx, cy, fw * 0.85)
        grad.setColorAt(0.0, qcol("#00141e"))
        grad.setColorAt(0.55, qcol("#000a11"))
        ...
        # ── state-aware colors (smoothly interpolated via self._col_cur) ──
        _halo_col = self._col_cur.name() if hasattr(self._col_cur, "name") else C.PRI
        ...
        p.setPen(QPen(qcol(_halo_col, sa), 2.5))
        ...
        p.setPen(QPen(qcol(_halo_col, 140), 1))
        ...
        p.setPen(QPen(qcol(_halo_col, int(self._halo * 0.5)), 1))
        ...
        if self._face_px:
            ...
            p.setPen(QPen(qcol(_halo_col, min(255, int(self._halo * 2))), 1))
        ...
        # ── hand-control: clap ripples ──
        for r in self._claps:
            a = max(0, min(255, int(200 * (1.0 - r / (fw * 0.6)))))
            if a <= 0:
                continue
        ...
        for g in self._gesture_fx:
            self._draw_gesture_badge(p, cx, cy - fw * 0.13, fw, g)
        ...
        if self.muted:
            ...
            sym = "◉" if self._blink else "◎"
            txt, col = f"{sym}  READY",      qcol(C.ACC)
        elif self.state == "OFFLINE":
            sym = "○" if self._blink else "○"
            txt, col = f"{sym}  OFFLINE",    qcol(C.TEXT_DIM)
        elif self.state == "LOCKED":
            sym = "⚠" if self._blink else "△"
            txt, col = f"{sym}  LOCKED",     qcol(C.RED)
        else:
            sym = "●" if self._blink else "○"
        ...
        for i in range(N):
            if self.muted:
                ...
                hgt = int(4 + 2 * math.sin(self._tick * 0.12 + i * 0.5))
                cl  = qcol(C.ACC, 180)
            elif self.state == "LOCKED":
                hgt = 2
                cl  = qcol(C.RED, 200)
                ...
                hgt = 1
                cl  = qcol(C.TEXT_DIM, 80)
            else:
                hgt = int(3 + 2 * math.sin(self._tick * 0.09 + i * 0.6))
                cl  = qcol(C.BORDER_B)
            p.fillRect(QRectF(wx0 + i * bw, wy + 20 - hgt, bw - 1, hgt), cl)

        # ── contextual activation hint (hand-control states) ──
        _hint = self._context_hint()
        if _hint:
            _ha = 140 + int(90 * (0.5 + 0.5 * math.sin(self._tick * 0.08)))
            p.setPen(QPen(qcol(_halo_col, _ha), 1))
    ...
    def _draw_gesture_badge(self, p, x: float, y: float, fw: float, g: dict) -> None:
        """Draw one animated gesture-recognition badge (sonar ping + emblem + label)."""
        try:
            now = time.time()
            age = now - g["t0"]
            life = 1.4
            if age < 0 or age > life:
                return
            pn = age / life
            fade = min(1.0, age / 0.12) * (1.0 - pn)
            if fade <= 0:
                return
            col = g["color"]
            cr, cg, cb = col.red(), col.green(), col.blue()
            rb = fw * 0.085
            s = 0.8 + 0.4 * math.sin(pn * math.pi)

            # sonar ping
            ping_r = rb * 1.2 + pn * fw * 0.20
            pa = max(0, int(160 * (1.0 - pn) * fade))
            if pa > 0:
                p.setPen(QPen(QColor(cr, cg, cb, pa), 2))
                p.setBrush(Qt.BrushStyle.NoBrush)
                p.drawEllipse(QRectF(x - ping_r, y - ping_r, ping_r * 2, ping_r * 2))

            br = rb * s
            # badge fill + border
            p.setBrush(QBrush(QColor(cr, cg, cb, max(0, int(60 * fade)))))
            p.setPen(QPen(QColor(cr, cg, cb, max(0, int(220 * fade))), 2))
            p.drawEllipse(QRectF(x - br, y - br, br * 2, br * 2))

            # finger-dot emblem
            n = g.get("dots", 3) or 0
            da = max(0, int(235 * fade))
            if n == 0:
                p.setBrush(QBrush(QColor(cr, cg, cb, da)))
                p.setPen(Qt.PenStyle.NoPen)
                p.drawEllipse(QPointF(x, y), max(2.0, br * 0.18), max(2.0, br * 0.18))
            else:
                spread = br * 1.1
                step = spread / max(1, n - 1) if n > 1 else 0.0
                for k in range(n):
                    off = (k - (n - 1) / 2.0) * step
                    p.setBrush(QBrush(QColor(cr, cg, cb, da)))
                    p.setPen(Qt.PenStyle.NoPen)
                    p.drawEllipse(QPointF(x + off, y), max(2.0, br * 0.16), max(2.0, br * 0.16))

            # name label
            name = (g.get("name") or "").replace("_", " ").title()
            p.setPen(QPen(QColor(cr, cg, cb, max(0, int(235 * fade))), 1))
            p.setFont(QFont(MONO, max(8, int(fw * 0.022)), QFont.Weight.Bold))
            p.drawText(QRectF(x - rb * 2.4, y + br + 4, rb * 4.8, 22),
                       Qt.AlignmentFlag.AlignCenter, name)

            # confidence arc
            conf = g.get("conf", 0.0)
            if conf > 0:
                p.setPen(QPen(QColor(cr, cg, cb, max(0, int(200 * fade))), 3))
                p.setBrush(Qt.BrushStyle.NoBrush)
                p.drawArc(QRectF(x - br, y - br, br * 2, br * 2),
                          int(90 * 16), int(-conf * 360 * 16))
        except Exception:
            pass
...
class _CameraPreview(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        title.setFont(QFont(MONO, 7, QFont.Weight.Bold))
        title.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        hdr.addWidget(title)
        hdr.addStretch()
        close_btn = QPushButton("✕")
        close_btn.setFixedSize(16, 16)
        close_btn.setFont(QFont(MONO, 8))
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        def _lbl(txt, font_size=9, bold=False, color=C.PRI,
            ...
            w.setFont(QFont(MONO, font_size,
        ...
        self._key_input.setFont(QFont(MONO, 10))
        ...
        for key, label in [("windows","⊞  Windows"),("mac","  macOS"),("linux","🐧  Linux")]:
            ...
            btn.setFont(QFont(MONO, 9, QFont.Weight.Bold))
        ...
        init_btn.setFont(QFont(MONO, 10, QFont.Weight.Bold))
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="JARVIS", user_name="",
        ...
        self._name_input.setFont(QFont(MONO, 10))
        ...
        self._user_input.setFont(QFont(MONO, 10))
        ...
        df_btn.setFont(QFont(MONO, 7, QFont.Weight.Bold))
        ...
        self._hex_input.setFont(QFont(MONO, 10))
        ...
        save_btn.setFont(QFont(MONO, 9, QFont.Weight.Bold))
        ...
        cancel_btn.setFont(QFont(MONO, 9))
...
class ClipboardPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        icon_lbl.setFont(QFont(MONO, 7, QFont.Weight.Bold))
        icon_lbl.setStyleSheet(f"color: {C.ACC2}; background: transparent;")
        hdr.addWidget(icon_lbl); hdr.addStretch()
        x_btn = QPushButton("✕")
        x_btn.setFixedSize(16, 16)
        x_btn.setFont(QFont(MONO, 8))
        ...
        self._preview.setFont(QFont(MONO, 8))
        ...
        for label, cmd_fmt in [
            ...
            b = QPushButton(label)
            b.setFixedHeight(22)
            b.setFont(QFont(MONO, 7, QFont.Weight.Bold))
...
class RemoteKeyOverlay(QWidget):
    ...
    def __init__(self, url: str, key: str, auto_login_url: str = "",
        ...
        self._url_lbl.setFont(QFont(MONO, 8))
        ...
        self._key_lbl.setFont(QFont(MONO, 28, QFont.Weight.Bold))
        ...
        self._timer_lbl.setFont(QFont(MONO, 8))
        ...
        new_btn.setFont(QFont(MONO, 8, QFont.Weight.Bold))
        ...
        close_btn.setFont(QFont(MONO, 8, QFont.Weight.Bold))
        close_btn.setCursor(Qt.CursorShape.PointingHandCursor)
    ...
    def _update_qr(self, url: str) -> None:
        ...
        try:
            ...
            self._qr_label.setFont(QFont(MONO, 8))
            self._qr_label.setStyleSheet(
                "color: #888; background: white; border-radius: 10px; padding: 4px;"
            )
        except Exception:
            self._qr_label.setText(url[:28])
            self._qr_label.setFont(QFont(MONO, 7))
            self._qr_label.setStyleSheet(
    ...
    def Jarvis_connected(self) -> None:
        ...
        """)
        self._qr_label.setText("✓")
        self._qr_label.setFont(QFont(MONO, 54, QFont.Weight.Bold))
        self._qr_label.setStyleSheet(
...
class _Toast(QWidget):
    """Transient bottom-centered notification — quick feedback without log spam."""
    ...
    def __init__(self, parent=None):
        ...
        lay.setSpacing(8)
        self._ico = QLabel("◈")
        self._ico.setFont(QFont(MONO, 11, QFont.Weight.Bold))
        self._ico.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        lay.addWidget(self._ico)
        self._msg = QLabel()
        self._msg.setFont(QFont(MONO, 9))
        self._msg.setStyleSheet(f"color: {C.TEXT}; background: transparent;")
    ...
    def show_msg(self, icon: str, msg: str, msecs: int = 2600) -> None:
...
class _CmdInput(QLineEdit):
    """Command input with ↑/↓ history recall."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self._hist: list[str] = []
        self._idx = 0
        self.setPlaceholderText("Type a command or question…")
        self.setFont(QFont(MONO, 9))
    ...
    def keyPressEvent(self, e):
        ...
        elif e.key() == Qt.Key.Key_Down and self._hist:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self.setWindowTitle(f"{_display} — Jarvis ")
        ...
        central.setStyleSheet(
            ...
            "stop:0 #05070d, stop:0.5 #070b14, stop:1 #04060c);"
        )
        ...
        _cam_title.setFont(QFont(MONO, 8, QFont.Weight.Bold))
        _cam_title.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        _cam_hdr.addWidget(_cam_title)
        _cam_hdr.addStretch()
        _cam_x = QPushButton("✕  CLOSE")
        _cam_x.setFont(QFont(MONO, 8, QFont.Weight.Bold))
        ...
        self._toast = _Toast(self.centralWidget())
    ...
    def _set_app_icon(self) -> None:
        """Ensure the arc-reactor .ico exists and apply it to the window/taskbar."""
    ...
    def resizeEvent(self, event):
        ...
        # Toast — bottom-center, just above the footer
    ...
    def _update_metrics(self):
        ...
        if _health_metrics is not None:
            try:
                _health_metrics.set("cpu", round(cpu, 1))
                _health_metrics.set("mem", round(mem, 1))
                _health_metrics.set("net_mbps", round(net, 2))
                if gpu >= 0:
                    _health_metrics.set("gpu", round(gpu, 1))
                if tmp >= 0:
                    _health_metrics.set("temp_c", round(tmp, 1))
                _health_metrics.set("proc_count", proc_count)
            except Exception:
                pass
    ...
    def _build_header(self) -> QWidget:
        ...
        w.setStyleSheet(
            ...
            f"stop:0 rgba(34,211,238,0.07), stop:1 rgba(7,11,20,0.72)); "
        ...
        self._drawer_btn.setFont(QFont(MONO, 11))
        ...
        self._title_lbl.setFont(QFont(MONO, 17, QFont.Weight.Bold))
        ...
        self._sub_lbl.setFont(QFont(MONO, 7))
        ...
        self._clock_lbl.setFont(QFont(MONO, 14, QFont.Weight.Bold))
        self._clock_lbl.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        self._clock_lbl.setAlignment(Qt.AlignmentFlag.AlignRight)
        right_col.addWidget(self._clock_lbl)
        self._date_lbl = QLabel("")
        self._date_lbl.setFont(QFont(MONO, 7))
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        w.setStyleSheet(f"background: rgba(8,13,24,0.55); border-right: 1px solid {C.BORDER};")
        lay = QVBoxLayout(w)
        lay.setContentsMargins(8, 10, 8, 10)
        lay.setSpacing(6)

        hdr = QLabel("◈ SYS MONITOR")
        hdr.setFont(QFont(MONO, 7, QFont.Weight.Bold))
        ...
        self._uptime_lbl.setFont(QFont(MONO, 8, QFont.Weight.Bold))
        self._uptime_lbl.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
        ip_lay.addWidget(self._uptime_lbl)

        self._proc_lbl = QLabel("PROC  --")
        self._proc_lbl.setFont(QFont(MONO, 8))
        self._proc_lbl.setStyleSheet(f"color: {C.TEXT_MED}; background: transparent; border: none;")
        ip_lay.addWidget(self._proc_lbl)

        os_name = {"Windows": "WIN", "Darwin": "macOS", "Linux": "LINUX"}.get(_OS, _OS.upper())
        os_lbl = QLabel(f"OS  {os_name}")
        os_lbl.setFont(QFont(MONO, 8))
        ...
        for txt, col in [
            ...
            lbl = QLabel(txt)
            lbl.setFont(QFont(MONO, 7, QFont.Weight.Bold))
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        w.setStyleSheet(f"background: rgba(8,13,24,0.55); border-left: 1px solid {C.BORDER};")
        lay = QVBoxLayout(w)
        ...
        def _sec(txt):
            l = QLabel(f"▸ {txt}")
            l.setFont(QFont(MONO, 7, QFont.Weight.Bold))
            l.setStyleSheet(
                f"color: {C.TEXT_MED}; background: transparent; "
                f"padding-left: 5px; border-left: 2px solid {C.PRI_DIM};"
            )
            return l
        ...
        self._file_hint.setFont(QFont(MONO, 7))
        ...
        lay.addLayout(self._build_chips())

        self._interrupt_btn = QPushButton("✋  INTERRUPT  [ESC]")
        self._interrupt_btn.setFixedHeight(34)
        self._interrupt_btn.setFont(QFont(MONO, 8, QFont.Weight.Bold))
        ...
        self._mute_btn.setFont(QFont(MONO, 8, QFont.Weight.Bold))
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        remote_btn.setFont(QFont(MONO, 8, QFont.Weight.Bold))
        ...
        self._autostart_btn.setFont(QFont(MONO, 7))
        ...
        self._brief_btn.setFont(QFont(MONO, 7))
    ...
    def _build_input_row(self) -> QHBoxLayout:
        ...
        send.setFont(QFont(MONO, 11, QFont.Weight.Bold))
    ...
    def _build_chips(self) -> QHBoxLayout:
        """One-tap quick actions — sends a natural-language command to JARVIS."""
        ...
        for icon, label, cmd in _chips:
            b = QPushButton(f"{icon} {label}")
            b.setFixedHeight(22)
            b.setFont(QFont(MONO, 7))
            b.setCursor(Qt.CursorShape.PointingHandCursor)
            b.setStyleSheet(f"""
                QPushButton {{
                    background: {C.PANEL2}; color: {C.TEXT_MED};
                    border: 1px solid {C.BORDER}; border-radius: 11px; padding: 0 7px;
                }}
                QPushButton:hover {{
                    color: {C.PRI}; border-color: {C.BORDER_B}; background: {C.PRI_GHO};
                }}
            """)
            b.clicked.connect(lambda _, c=cmd: self._send_text(c))
            row.addWidget(b)
        return row
    ...
    def _build_content_panel(self) -> QWidget:
        ...
        dot = QLabel("◈")
        dot.setFont(QFont(MONO, 9, QFont.Weight.Bold))
        dot.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        hdr.addWidget(dot)

        self._content_title_lbl = QLabel("BRIEFING")
        self._content_title_lbl.setFont(QFont(MONO, 8, QFont.Weight.Bold))
        ...
        self._content_ts_lbl.setFont(QFont(MONO, 7))
        self._content_ts_lbl.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent;")
        hdr.addWidget(self._content_ts_lbl)

        dismiss = QPushButton("DISMISS  ✕")
        dismiss.setFont(QFont(MONO, 7))
        ...
        self._content_display.setFont(QFont(MONO, 8))
    ...
    def _show_content(self, title: str, text: str):
        ...
        self._content_display.setHtml(markdown_to_html(text))
    ...
    def _build_footer(self) -> QWidget:
        ...
        def _fl(txt, color=C.TEXT_MED):
            l = QLabel(txt); l.setFont(QFont(MONO, 7))
            l.setStyleSheet(f"color: {color}; background: transparent;")
            return l
    ...
    def _on_file_selected(self, path: str):
        ...
        self.show_toast("📎", f"File loaded — {p.name}  ({size})")
    ...
    def notify_phone_connected(self) -> None:
        if self._remote_overlay and self._remote_overlay.isVisible():
            self._remote_overlay.Jarvis_connected()
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self.setWindowTitle(f"{display} — Jarvis ")
    ...
    def show_toast(self, icon: str, msg: str):
        self._toast_sig.emit(icon, msg)

    def _do_toast(self, icon: str, msg: str):
        self._toast.show_msg(icon, msg)
        self._position_toast()

    def _position_toast(self):
        cw = self.centralWidget()
        pw = self._toast.width()
        ph = self._toast.height()
        x = (cw.width() - pw) // 2
        y = cw.height() - ph - 28
        self._toast.setGeometry(x, y, pw, ph)
        self._toast.raise_()
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        self.show_toast("◈", f"{self._assistant_name} online — OS: {os_name.upper()}")
...
class JarvisUI:
    ...
    def toast(self, icon: str, msg: str):
        """Thread-safe: show a transient notification (e.g. ui.toast('🌤', '...'))."""
        self._win._toast_sig.emit(icon, msg)
    ...
    def show_gesture_feedback(self, gesture: str, confidence: float = 0.0) -> None:
        """Thread-safe: show an animated gesture badge on the HUD orb."""
        try:
            self._win.hud.gesture_feedback(gesture, confidence)
        except Exception:
            self._win._toast_sig.emit("◈", f"{gesture} recognized")
